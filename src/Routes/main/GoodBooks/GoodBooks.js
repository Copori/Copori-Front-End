...
import { GiSpellBook, GiBookmarklet, GiSecretBook, GiBlackBook } from 'react-icons/gi';
...
function GoodBooks() {
  ...
  const getBooks = async () => {
    ...
    for (let i = 1; i < 7; i++) {
      const json = await (
        await fetch(`${API_URL}?serviceKey=${SECRET_KEY}&numOfRows=${countBook}&pageNo=${pageNum + i}`, {
          headers: {
    ...
    // 전체 도서 Data
    const json = await (
      await fetch(`${API_URL}?serviceKey=${SECRET_KEY}&numOfRows=825&pageNo=1`, {
        headers: {
          accept: 'application/json',
        },
      })
    ...
    <div className="GoodBooks">
      <div className="GoodBooks__title">
        <div className="GoodBooks__title--left">
          <div className="GoodBooks__title--icon" onClick={onRecommandBookToggle}>
